...
if [ ! -e "$HOME/.openvscodeserver/bin" ]; then
    ...
    curl -fsSL https://github.com/gitpod-io/openvscode-server/releases/download/openvscode-server-v1.75.0/openvscode-server-v1.75.0-linux-x64.tar.gz -o /tmp/openvscode-server.tar.gz
...
if [ "$(ps -ef | grep '\.openvscode-server' | wc -l)" = "1" ]; then
    ...
    "$HOME"/.openvscodeserver/bin/openvscode-server serve-local --without-connection-token --host 0.0.0.0 --port 26000
else
    echo -e "\ncode-server is already running.\n\nConnect using: http://localhost:26000\n"
fi